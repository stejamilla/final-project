...
We recognize that this method is imperfect. If, for example, if within a ward, we had to impute data for a couple block groups based on tract-level averages and then impute data for other block groups based on the ward-level average, then that means the ward-level average for a block group was created partially by other already-imputed values (using tract-level averages). However, there were only a small number of instances of these.* Another limitation is that median househould income and median property value variables are top- and bottom-coded, meaning that imputed values (based on averages) may be lower or higher than they should be due to the top/bottom coding. 

*STEPHANIE 12/16 - Could someone check how many instances there were? I wrote the above based on Madeleine's notes.
...
```
STEPHANIE 12/16 - I'm a bit bewildered how the regression model correctly predicts ALL uhi values when rounded to the nearest tenth...is this possible?! When you round to the nearest hundreth, it still gets 109 of 116 correct. But then by the thousandth it goes to only 73 correct.
