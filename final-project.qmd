...
JAMIE 12/3
I like this idea! That makes sense! Are you thinking we'll use the geometry in the landcover dataset? Or were you thinking of just using the data per block group?

MADELEINE 12/4
agreed!

Stephanie 12/4
We need the geometry column to make any mapping data visualizations but we should drop the geometry when actually doing supervised machine learning
...
MADELEINE 12/4 - good w all these steps!
...
JAMIE 12/3 - I keep getting multiple duplicates, with an additional block_group column being added, and the block groups not matching. Stuck on this error. I think it's because the geometry in landcover doesn't align with that of dc_heat?
```{r}
...
# when we join these, it creates about 6 dupes for each block group with mismatched second block_group column
dataset_final <- st_join(dc_heat, dataset_interim)
...
MADELEINE 12/4
This map looks a bit odd to me. It's saying the hottest part of DC is right around the White House? Which is an area with a decent amount of parks and greenery. It also doesn't seem to match [other heat maps](https://www-washingtonpost-com.proxy.library.georgetown.edu/weather/2018/10/15/sizzling-summer-days-northeast-dc-heats-up-most-noaa-analysis-shows/er/2018/10/15/sizzling-summer-days-northeast-dc-heats-up-most-noaa-analysis-shows/) I found online, which tend to show NE as the hottest area.

Not sure how important it is for this variable to be accurate, but if it is, I'm thinking maybe we should dig into it a bit more.

STEPHANIE 12/4
It may have to do with the different methodology between maps? Unsure, maybe we should bring this up to Aaron.
...
dataset_dt <- dataset_final |>
  st_drop_geometry() |>
  select(-uhi, -block_group.x, -block_group.y) 
#dropping uhi because that would be the main decision tree factor

```
MADELEINE 12/4
I am getting an error in the last line of the above chunk because there is no variable "blockgroup" in dataset_final. Not removing it from the code since I'm not sure what other variable you meant to exclude
Stephanie 12/4
Whoops, that was because when I was exploring the data things were getting messy for me and I accidentally had that column.
