...
Reminder of Climate Central's definition of UHI: "UHI index values (in °F) are estimates of how much the urban built environment boosts temperatures. In other words, the UHI index is an estimate of the additional heat that local land use factors contribute to urban areas."
...
names(landcover) <- names(landcover) |>
  tolower()
landcover$block_group <- as.character(landcover$block_group)
...
Selecting only the percentage data:
```{r}
...
# Getting the data
url <- "https://api.census.gov/data/2020/acs/acs5?get=NAME,B01003_001E,B19001_002E,B19001_003E,B19001_004E,B19001_005E,B19001_006E,B19001_007E,B19001_008E,B19001_009E,B19001_010E,B19001_011E,B19001_012E,B19001_013E,B19001_014E,B19001_015E,B19001_016E,B19001_017E&for=block%20group:*&for=tract:*&in=state:11&in=county:001"
...
# Creating block_group column to use for combining this with the other datasets
acs_data$block_group <- paste(acs_data$state, 
                             acs_data$county, 
                             acs_data$tract, 
                             acs_data$block_group_temp,
                             sep = "")
...
Note: We should do some data cleaning on dataset_final before running the decision tree but it's getting late and I am tired, so I'm leaving it as is for now -Stephanie
```{r}

set.seed(20241202)

dt_split <- initial_split(data = dataset_final, prop = 0.8)
dt_train <- training(x = dt_split) 
dt_test <- testing(x = dt_split)

dt_recipe <- recipe(formula = hotspot ~ ., data = grades_train) |>
...
rpart.plot::rpart.plot(x = fitting$fit$fit$fit,
                       roundint = FALSE)
