...
kable(acs_codes_summary <- acs_data |>
  ...
  summarise(across(everything(), list(
    `sum-666666666` = ~sum(. == "-666666666", na.rm=T),
    `sum-999999999` = ~sum(. == "-999999999", na.rm=T),
    `sum-888888888` = ~sum(. == "-888888888", na.rm=T),
    `sum-222222222` = ~sum(. == "-222222222", na.rm=T),
    `sum-333333333` = ~sum(. == "-333333333", na.rm=T),
    `sum-555555555` = ~sum(. == "-555555555", na.rm=T),
    `sum*` = ~sum(. == "*", na.rm=T),
...
acs_data <- acs_data |> 
  mutate(across(where(is.character), 
                ~if_else(. %in% acs_codes_char, NA_character_, .))) |>
