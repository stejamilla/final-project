...
url <- str_glue(paste0("https://api.census.gov/data/2020/acs/acs5?get=",
             ...
             # MEDIAN GROSS RENT
             "B25064_001E,",
             # MEDIAN GROSS RENT AS A PERCENTAGE OF HOUSEHOLD INCOME IN THE
             # PAST 12 MONTHS
             "B25071_001E",
...
MADELEINE 12/14 - I figured this out and fixed it; we were pulling the total population from the ethnicity table, which is just equal to the total population overall. It should now be pulling the actual Hispanic/Latino count
...
MADELEINE 12/14: I am trying a new approach using 1 standard deviation above the mean, which makes the cutoff temp 91.6 F.

I've taken care of all the NaN values but haven't dealt with the NA values which I've found in the following columns: med_hhi, med_prop_val, med_gross_rent, med_rent_pct_inc, income_home_val_ratio.

JAMIE 12/13 - Just to make sure we're all on the same page, some values of the variables med_hhi, med_prop_val, med_gross_rent, and med_rent_pct_inc have NAs because these entries had error coding "-666666666" - see table in code chunk #8. I recoded them to NA.

Description of "-666666666" error code from the Census: (http://bit.ly/3DnlfIh). NOTE for our 5-yr estimate data, this appears to be an MOE issue (see last sentence):
...
JAMIE 12/12 - Changing the dataset used in the recipe to dc_train rather than the full df
