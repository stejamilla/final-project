...
JAME 12/12 - I fixed this - I guess we have to left_join with the sf df in the first position in left_join(), so that we join the non-sf df onto the sf df and it stays sf. BUT do we have to st_transform() since the crs is not 4326? Also, when I move the code to remove uhi BEFORE we do the training set, that means we can't run this map. Can we remove uhi afterward?
...
JAMIE 12/12 - Now getting an error regarding the number of datapoints in cumulative...will have another look tomorrow. Do the numbers need to change since this is training data only?
...
# Having trouble with a factor error below that is over my head...will continue to work on it...
...
JAMIE 12/12 - I do not really know what numbers to put for things like trees etc at the moment...just putting in what I used for the homework from class examples, for us to update later!

* another error here I need to work through...
...
test_predictions <-
  ...
  mutate(y_hat = as.numeric(as_vector(predict(best_lm_fit, dc_modeling_test))))
