...
acs_variables_tract <- load_variables(2020, "acs5") |>
...
STEPHANIE 12/15 - I've set up code to evaluate imputation from lines 348-372.
...
mean_imputed_hhi <- acs_data |>
  left_join(tract_data, by = "tract") |>
  mutate(
    hhi_imputation = as.numeric(med_hhi < 0),
    hhi_imputed = if_else(
      med_hhi < 0,
      hhi_tract,
      med_hhi
  ...
  select(med_hhi, hhi_imputation, hhi_imputed) |>
  # Replacing -666666666 values with NA for the sake of data visualization
  mutate(med_hhi = if_else(
    med_hhi < 0,
    NA,
    med_hhi
...
mean_imputed_hhi |>
  ggplot(aes(med_hhi, hhi_imputed, fill = factor(hhi_imputation))) +
...
It looks like mtry = 15 and min_n = 4 are the best options.
